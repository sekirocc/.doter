...
STRING is the message to display."
  ...
  (cond ((or (not eldoc-on-hold--use-timer) (and string eldoc-last-message))
        ...
        ((not string)
         ...
         (funcall orig-fun string))
        (t
          (message "run-with-timer %s, %s, %s" orig-fun string eldoc-on-hold-delay-interval)
...
(defun eglot-on-hold--msg (orig-fun)
  "Show eglot highlight with a delay.
Used to advice ORIG-FUN, which should be 'eglot-hover-eldoc-function' "
  (when eldoc-on-hold--msg-timer
    (cancel-timer eldoc-on-hold--msg-timer)
    (setq eldoc-on-hold--msg-timer nil))
  (message "run-with-timer %s, %s" orig-fun eldoc-on-hold-delay-interval)
  (setq eldoc-on-hold--msg-timer
    (run-with-timer eldoc-on-hold-delay-interval nil orig-fun))
  )
